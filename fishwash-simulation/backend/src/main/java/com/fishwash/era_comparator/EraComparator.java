...
public class EraComparator {
    ...
    public CrossEraComparisonResult compareAncientVsModern(Integer ancientDeviceId) {
    ...
    public CompletableFuture<CrossEraComparisonResult> compareAncientVsModernAsync(Integer ancientDeviceId) {
        return CompletableFuture.completedFuture(compareAncientVsModern(ancientDeviceId));
    }

    public static double calculateEnergyEfficiencyRatio(double ancientFreqHz, double ancientSprayCm,
                                                         double modernFreqHz, double modernPowerW,
                                                         double modernOutputMlH) {
        double ancientFlow = Math.max(1.0, ancientSprayCm * 0.45);
        double modernFlow = modernOutputMlH / 60.0;
        double ancientJoulesPerMl = ancientFreqHz * 0.00042 / Math.max(0.1, ancientFlow);
        double modernJoulesPerMl = modernPowerW / Math.max(0.1, modernFlow);
        return ancientJoulesPerMl / Math.max(1e-6, modernJoulesPerMl);
    }
}
