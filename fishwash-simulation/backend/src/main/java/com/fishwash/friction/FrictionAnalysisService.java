...
public class FrictionAnalysisService {
    ...
    public FrictionAnalysis analyzeFrictionMechanics(Integer deviceId, FrictionAnalysisRequest req) {
        ...
        double effectiveMu = Math.max(0.08, mu * (1.0 - waterLubrication * 0.4));
        ...
        double stickSlipFreq = (1.0 / (2.0 * Math.PI)) * Math.sqrt(k_stick /
                (skinAreaCm2 * 1e-4 * 2.0 * fishWashProperties.getMaterial()
                        .getProfiles().getOrDefault("bronze-standard",
                                new FishWashProperties.MaterialProfile())
                        .getDensity() * 0.003));
    ...
    private Map<String, Object> buildStickSlipThresholdMap(double N, double muS, double muK, double v) {
    ...
    private Map<String, Object> buildFreeBodyDiagram(double N, double muEff, double v, double R_handle) {
    ...
    public org.springframework.data.domain.Page<FrictionAnalysis> getAnalysisPaged(
